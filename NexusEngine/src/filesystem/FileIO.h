...
namespace NexusEngine::IO
{
    ...
    NEXUS_ENGINE_API std::optional<std::vector<std::uint8_t>> ReadFileBytes(const std::filesystem::path& filePath);
    ...
    NEXUS_ENGINE_API bool WriteFileBytes(const std::filesystem::path& filePath, const std::uint8_t* bytes, std::size_t byteCount);
    ...
    /// <returns>True if the file was written; otherwise false.</returns>
    NEXUS_ENGINE_API bool WriteFileBytes(const std::filesystem::path& filePath, const std::vector<std::uint8_t>& bytes);
    ...
    NEXUS_ENGINE_API bool WriteStructuredFile(const std::filesystem::path& filePath, FileFormat fileFormat, const StructuredWriteMethod& writeMethod);
    ...
    NEXUS_ENGINE_API bool ReadStructuredFile(const std::filesystem::path& filePath, FileFormat fileFormat, const StructuredReadMethod& readMethod);
